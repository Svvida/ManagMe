        ...
        <CardContent sx={{ flexGrow: 1, p: 3 }}>
          <Box sx={{ display: 'flex', alignItems: 'flex-start', mb: 2 }}>
            ...
            <Box sx={{ flexGrow: 1, minWidth: 0 }}>
              ...
              {/* FIX: Added Tooltip to show full ID on hover */}
          ...
          <Typography variant="body2" color="text.secondary" sx={{ display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden', lineHeight: 1.5, minHeight: '4.5em' }}>
            {project.description || 'No description provided'}
          </Typography>
        </CardContent>
        <CardActions sx={{ p: 2, pt: 0, justifyContent: 'flex-end' }}>
          <Tooltip title="Edit project">
            <IconButton onClick={() => onEdit(project)} sx={{ color: '#673ab7', '&:hover': { backgroundColor: alpha('#673ab7', 0.1) } }}>
              <Edit fontSize="small" />
            </IconButton>
          </Tooltip>
          <Tooltip title="Delete project">
            <IconButton onClick={() => onDelete(project.id)} disabled={isDeleting} sx={{ color: 'error.main', '&:hover': { backgroundColor: alpha('#f44336', 0.1) } }}>
